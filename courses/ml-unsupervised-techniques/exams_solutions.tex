...
For a discrete random variable, an expectation is a probability-weighted sum; for a
continuous random variable with density $f_X$, it is an integral:
\[
\E[g(X)]
=\sum_x g(x)P(X=x)
\quad\text{or}\quad
...
\]
To compute a moment, choose $g(x)=x^r$. Since $X\sim U[-1,1]$, its density is
\[
f_X(x)=\frac{1}{2}\quad\text{for }-1\leq x\leq1
\qquad\text{and}\qquad
f_X(x)=0\quad\text{otherwise}.
\]
Consequently,
\[
\E(X^r)=\int_{-1}^{1}x^r\frac12\dd x.
\]
Odd moments are zero because an odd integrand cancels on the symmetric interval. For even
moments $r=2k$,
\[
\E(X^{2k})
=\frac12\left[\frac{x^{2k+1}}{2k+1}\right]_{-1}^{1}
=\frac{1}{2k+1}.
\]
In particular,
\[
\E(X^2)=\frac12\left[\frac{x^3}{3}\right]_{-1}^{1}
=\frac13,
\qquad
\E(X^4)=\frac12\left[\frac{x^5}{5}\right]_{-1}^{1}
=\frac15.
\]
Also $\E(X)=0$ by symmetry, so $X$ is centered and the given cumulant formula applies:
\[
\kappa_4(X)=\tfrac15-3\bigl(\tfrac13\bigr)^2=\tfrac15-\tfrac13=-\tfrac{2}{15}\approx\mathbf{-0.13}.
\]
Cumulants of order $n$ scale as $\kappa_n(cX)=c^n\kappa_n(X)$. Therefore,
\[
\kappa_4(2X)=2^4\kappa_4(X)
=16\left(-\frac{2}{15}\right)
=-\frac{32}{15}\approx\mathbf{-2.13}.
\]
For independent variables, cumulants of the same order add. Since $X$ and $Y$ have the
same distribution,
\[
\kappa_4(X+Y)=\kappa_4(X)+\kappa_4(Y)
=2\left(-\frac{2}{15}\right)
=-\frac{4}{15}\approx\mathbf{-0.27}.
\]
...
\]
Here the only possible values are $-1$ and $1$, each with probability $\tfrac12$. Thus,
\[
\E(X)=(-1)\frac12+(1)\frac12=0,
\]
\[
\E(X^2)=(-1)^2\frac12+1^2\frac12=1,\qquad
\E(X^3)=(-1)^3\frac12+1^3\frac12=0,
\]
\[
\E(X^4)=(-1)^4\frac12+1^4\frac12=1.
\]
Equivalently, $X^2=X^4=1$ always and $X^3=X$. Because $\E(X)=0$, $X$ is centered, so
\[
...
\]
Using homogeneity of fourth-order cumulants,
\[
\frac{\kappa_4(3X)}{\kappa_4(X)}
=\frac{3^4\kappa_4(X)}{\kappa_4(X)}
=3^4=\mathbf{81}.
\]
Using additivity for the independent variables $X$ and $Y$ and the fact that they are
identically distributed,
\[
\frac{\kappa_4(X+Y)}{\kappa_4(X)}
=\frac{\kappa_4(X)+\kappa_4(Y)}{\kappa_4(X)}
=\frac{2\kappa_4(X)}{\kappa_4(X)}
=\mathbf{2}.
\]
\end{answerprose}
...
For a real symmetric covariance matrix, eigenvectors belonging to different eigenvalues are
orthogonal. Together with the given normalization this means
$v_i^\top v_j=\delta_{ij}$: the inner product is $1$ when $i=j$ and $0$ otherwise.
Also, by the definition of an eigenvector, $Cv_i=\lambda_i v_i$. First multiply from the
right:
\[
C(3v_2+v_3)
=3Cv_2+Cv_3
=3\lambda_2v_2+\lambda_3v_3
=9v_2+v_3.
\]
Now expand all products:
\[
\begin{aligned}
A
&=(2v_1^\top+v_2^\top+3v_3^\top)(9v_2+v_3)\\
&=18\,v_1^\top v_2+2\,v_1^\top v_3
  +9\,v_2^\top v_2+v_2^\top v_3
  +27\,v_3^\top v_2+3\,v_3^\top v_3\\
&=0+0+9+0+0+3=\mathbf{12}.
\end{aligned}
\]
\end{answerprose}
...
In general, the integral only has to cover the support of the first distribution $p_Y$,
because the factor $p_Y(x)$ is zero elsewhere. Here this support is $[0,\theta_Y]$, and it
lies inside $[0,\theta_Z]$ because $e^5<e^{10}$. On this interval both densities are
nonzero and their ratio is constant:
\[
\begin{aligned}
D_{\mathrm{KL}}(p_Y\|p_Z)
&=\int_0^{\theta_Y}\frac{1}{\theta_Y}
  \ln\!\left(\frac{1/\theta_Y}{1/\theta_Z}\right)\dd x\\
&=\ln\!\left(\frac{\theta_Z}{\theta_Y}\right)
  \underbrace{\int_0^{\theta_Y}\frac{1}{\theta_Y}\dd x}_{=\,1}\\
&=\ln\!\left(\frac{e^{10}}{e^5}\right)
 =\ln(e^5)=\mathbf{5.00}.
\end{aligned}
\]
...
An outer product multiplies every component of the column vector $x$ with every component
of the row vector $y^\top$. In symbols, $(xy^\top)_{ij}=x_i y_j$. Therefore,
\[
A=xy^\top
...
\end{pmatrix}.
\]
Hence the requested third row is
\[
(\mathbf{12},\,\mathbf{15},\,\mathbf{18}).
\]
\end{answerprose}
...
Marginal probabilities are obtained by summing the joint distribution over the other
variable:
\[
P(X=x)=\sum_y P(X=x,Y=y),\qquad
P(Y=y)=\sum_x P(X=x,Y=y).
\]
Thus the complete marginals are
\[
P(X=0)=0.25+0.125=0.375,\qquad
P(X=1)=0.25+0.375=0.625\approx\mathbf{0.63},
\]
\[
P(Y=0)=0.25+0.25=0.50,\qquad
P(Y=1)=0.125+0.375=\mathbf{0.50}.
\]
For a discrete random variable, $H(X)=-\sum_x p_X(x)\ln p_X(x)$. Therefore,
\[
\begin{aligned}
H(X)
&=-\bigl(0.375\ln 0.375+0.625\ln 0.625\bigr)\\
&\approx-\bigl(0.375(-0.981)+0.625(-0.470)\bigr)
 \approx\mathbf{0.66},
\end{aligned}
\]
\[
H(Y)=-\bigl(0.5\ln0.5+0.5\ln0.5\bigr)=\ln2\approx\mathbf{0.69}.
\]
The joint entropy uses all four cells:
\[
\begin{aligned}
H(X,Y)
&=-\sum_{x,y} p(x,y)\ln p(x,y)\\
&=-\bigl(0.25\ln0.25+0.125\ln0.125
          +0.25\ln0.25+0.375\ln0.375\bigr)\\
&\approx1.321.
\end{aligned}
\]
Finally, mutual information is the amount by which knowing one variable reduces the
uncertainty about the other:
\[
I(X;Y)=H(X)+H(Y)-H(X,Y)
\approx0.662+0.693-1.321
\approx\mathbf{0.03}.
\]
\end{answerprose}
...
Add rows or columns of the joint table to obtain the marginals:
\[
P(X=0)=\tfrac13+\tfrac13=\tfrac23,\qquad
P(X=1)=0+\tfrac13=\tfrac13,
\]
\[
P(Y=0)=\tfrac13+0=\tfrac13\approx\mathbf{0.33},\qquad
P(Y=1)=\tfrac13+\tfrac13=\tfrac23.
\]
Using $H(X)=-\sum_x p_X(x)\ln p_X(x)$, both variables have the same entropy because both
have marginal probabilities $\{\tfrac13,\tfrac23\}$:
\[
H(X)=H(Y)
=-\left(\tfrac23\ln\tfrac23+\tfrac13\ln\tfrac13\right)
\approx\mathbf{0.64}.
\]
The joint distribution has three nonzero cells, each with probability $\tfrac13$.
The zero cell contributes $0\ln0:=0$, so
\[
H(X,Y)=-3\left(\tfrac13\ln\tfrac13\right)=\ln3\approx1.099.
\]
Therefore,
\[
I(X;Y)=H(X)+H(Y)-H(X,Y)
\approx0.637+0.637-1.099
\approx\mathbf{0.17}.
\]
\end{answerprose}
...
Summing the rows and columns gives all marginal probabilities:
\[
P(X=0)=0.3+0.4=\mathbf{0.70},\qquad
P(X=1)=0.2+0.1=0.30,
\]
\[
P(Y=0)=0.3+0.2=\mathbf{0.50},\qquad
P(Y=1)=0.4+0.1=0.50.
\]
Insert these marginals into $H(X)=-\sum_x p_X(x)\ln p_X(x)$:
\[
H(X)=-(0.7\ln 0.7+0.3\ln 0.3)\approx\mathbf{0.61},\qquad
H(Y)=-2(0.5\ln0.5)=\ln 2\approx\mathbf{0.69}.
\]
For the joint entropy, use the four entries of the original table:
\[
\begin{aligned}
H(X,Y)
&=-\sum_{x,y} p(x,y)\ln p(x,y)\\
&=-\bigl(0.3\ln0.3+0.4\ln0.4+0.2\ln0.2+0.1\ln0.1\bigr)\\
&\approx1.280.
\end{aligned}
\]
Hence,
...
\begin{answer}{A, E, F}
  ...
  \no{G} A likelihood is not, in general, the probability of the observed data. More
  precisely, $\mathcal L(w;x)=p(x\mid w)$ is the probability mass or density at the
  observed $x$, viewed as a function of the parameter $w$. It need not sum or integrate
  to $1$ over $w$; for continuous data, moreover, $P(X=x)=0$ even when
  $f(x\mid w)>0$. Therefore the wording ``the probability that the model produces the
  data'' is false.
...
\begin{itemize}[leftmargin=1.7em,itemsep=1.5pt,topsep=2pt]
  ...
  \item Let $\E(X_i)=\mu$ and $\Var(X_i)=\sigma^2$. For
        $\hat\mu_n=\frac{1}{n+1}\sum_{i=1}^nX_i$, linearity of expectation gives
        \[
        \E(\hat\mu_n)
        =\frac{1}{n+1}\sum_{i=1}^n\E(X_i)
        =\frac{n}{n+1}\mu\neq\mu
        \]
        for finite $n$, so the estimator is biased. However,
        \[
        \operatorname{bias}(\hat\mu_n)
        =-\frac{\mu}{n+1}\longrightarrow0,\qquad
        \Var(\hat\mu_n)
        =\frac{n\sigma^2}{(n+1)^2}\longrightarrow0.
        \]
        Thus it converges to $\mu$ and is \textbf{biased but consistent}.
  \item Taking $\hat\mu=X_1$ gives $\E(\hat\mu)=\mu$, so it is unbiased. But
        $\Var(\hat\mu)=\sigma^2$ for every sample size: the variance never approaches zero
        because all further observations are ignored. Therefore $X_1$ is
        \textbf{unbiased but not consistent}.
...
Put the eigenvectors into the columns of $U=(u_1,u_2,u_3)$. The PCA coordinates are
\[
Y=XU,\qquad
Y_{ik}=x_i^\top u_k.
\]
Thus every entry is the scalar projection of sample $x_i$ onto direction $u_k$. For the
second sample $x_2=(2,2,2)^\top$:
\[
...
\]
Repeating the same three scalar products for all four rows gives the complete projection
matrix:
\[
Y=
\begin{pmatrix}
...
\end{pmatrix}.
\]
Every row of $X$ is a multiple of $(1,1,1)^\top$: the matrix has rank $1$, so all
variation lies along $u_1$ and the other two coordinates vanish for every sample. The
requested row is therefore
\[
y_2=(\mathbf{3.46},\,\mathbf{0.00},\,\mathbf{0.00}).
\]
\end{answerprose}
...
Again, $U=(u_1,u_2,u_3)$ and $Y=XU$, so $Y_{ik}=x_i^\top u_k$. For
$x_3=(-3,0,3)^\top$:
\[
...
\]
For a general row $x_i=(-i,0,i)^\top$, these products are
$x_i^\top u_1=i\sqrt2$ and $x_i^\top u_2=x_i^\top u_3=0$. Hence the complete matrix is
\[
Y=
\begin{pmatrix}
\sqrt2&0&0\\
...
\end{pmatrix}.
\]
Thus $y_3=(\mathbf{4.24},\,\mathbf{0.00},\,\mathbf{0.00})$. All rows are multiples
of $(-1,0,1)^\top$, so the data is one-dimensional along $u_1$.
\end{answerprose}
...
\closequestion
\begin{answerprose}{(4.24, 0.00, 0.00)}
Using the projection convention in the question, put the stated directions into
$U=(u_1,u_2,u_3)$ and calculate $Y=XU$. For $x_3=(3,0,3)^\top$:
\[
...
y_3^{(3)}=\mathbf{0}.
\]
The other rows are computed in exactly the same way:
\[
Y=
\begin{pmatrix}
-4\sqrt2&0&0\\
...
\end{pmatrix}.
\]
The third sample lies exactly on the $(1,0,1)$ direction, so only its first component
survives:
\[
y_3=(\mathbf{4.24},\,\mathbf{0.00},\,\mathbf{0.00}).
\]
...
\begin{itemize}
  ...
  \choice Criteria for independence are mutual information between components and\\
...
In general, entry $(i,j)$ of $UU^\top$ is the dot product of row $i$ and row $j$ of
$U$:
\[
(UU^\top)_{ij}=\sum_{k=1}^{\ell}U_{ik}U_{jk}.
\]
For the first row, take $(1,2)$ and form its dot product with each row of $U$:
\[
(UU^\top)_{11}=1\cdot1+2\cdot2=5,\qquad
(UU^\top)_{12}=1\cdot3+2\cdot4=11,\qquad
(UU^\top)_{13}=1\cdot5+2\cdot6=17.
\]
Hence the first row of $UU^\top$ is $(5,11,17)$. The first row of $\Psi$ is
$(5,0,0)$, so
\[
C_{1,:}=(5,11,17)+(5,0,0)
=(\mathbf{10},\,\mathbf{11},\,\mathbf{17}).
\]
The diagonal noise term changes $C_{11}$ but not the off-diagonal entries in this row.
\end{answerprose}
...
\textbf{Assignment step.} For $x=(x_1,x_2)$ and center
$c=(c_1,c_2)$, use
\[
d^2(x,c)=(x_1-c_1)^2+(x_2-c_2)^2.
\]
The square root is unnecessary because the smaller squared distance also has the smaller
ordinary distance. With $\mu=(0,1)$ and $\nu=(2,2)$:
\[
\begin{array}{c|cc|c}
x & d^2(x,\mu) & d^2(x,\nu) & \text{assignment}\\
\hline
...
\textbf{Update step.} A new center is the component-wise mean of all points assigned to
that cluster:
\[
\begin{aligned}
\mu^{\mathrm{new}}
&=\tfrac14\bigl[(1,0)+(0,1)+(-1,0)+(0,-1)\bigr]\\
&=\left(\frac{1+0-1+0}{4},\frac{0+1+0-1}{4}\right)
 =(\mathbf{0},\mathbf{0}),
\end{aligned}
\]
\[
\begin{aligned}
\nu^{\mathrm{new}}
&=\tfrac13\bigl[(2,0)+(2,3)+(5,0)\bigr]\\
&=\left(\frac{2+2+5}{3},\frac{0+3+0}{3}\right)
 =(\mathbf{3},\mathbf{1}).
\end{aligned}
\]
\end{answerprose}
...
\textbf{Assignment step.} Compare
$d^2(x,c)=(x_1-c_1)^2+(x_2-c_2)^2$ for the two initial centers:
\[
\begin{array}{c|cc|c}
x & d^2(x,\mu) & d^2(x,\nu) & \text{assignment}\\
\hline
...
\textbf{Update step.} Take the component-wise mean in each cluster:
\[
\begin{aligned}
\mu^{\mathrm{new}}
&=\tfrac14\bigl[(1,0)+(2,-1)+(1,-2)+(0,-1)\bigr]\\
&=\left(\frac{1+2+1+0}{4},\frac{0-1-2-1}{4}\right)
 =(\mathbf{1},\mathbf{-1}),
\end{aligned}
\]
\[
\begin{aligned}
\nu^{\mathrm{new}}
&=\tfrac14\bigl[(2,0)+(2,1)+(3,0)+(3,1)\bigr]\\
&=\left(\frac{2+2+3+3}{4},\frac{0+1+0+1}{4}\right)
 =(\mathbf{2.5},\mathbf{0.5}).
\end{aligned}
\]
\end{answerprose}
...
\textbf{Assignment step.} Compare the squared Euclidean distances to
$\mu=(0,0)$ and $\nu=(2,1)$:
\[
\begin{array}{c|cc|c}
x & d^2(x,\mu) & d^2(x,\nu) & \text{assignment}\\
...
\textbf{Update step.} Average the coordinates of the assigned points:
\[
\begin{aligned}
\mu^{\mathrm{new}}
&=\tfrac14\bigl[(1,-1)+(0,0)+(-1,-1)+(0,-2)\bigr]\\
&=\left(\frac{1+0-1+0}{4},\frac{-1+0-1-2}{4}\right)
 =(\mathbf{0},\mathbf{-1}),
\end{aligned}
\]
\[
...
\textbf{Assignment step.} Use
$d^2(x,c)=(x_1-c_1)^2+(x_2-c_2)^2$ and compare the squared distances to
$\mu=(2,0)$ and $\nu=(-1,4)$:
\[
\begin{array}{c|cc|c}
x & d^2_\mu & d^2_\nu & \to\\
...
\begin{aligned}
\mu^{\mathrm{new}}
&=\tfrac14\bigl[(0,1)+(1,2)+(2,1)+(1,0)\bigr]\\
&=\left(\frac{0+1+2+1}{4},\frac{1+2+1+0}{4}\right)
...
\begin{aligned}
\nu^{\mathrm{new}}
&=\tfrac14\bigl[(0,2)+(-1,2)+(0,3)+(-1,3)\bigr]\\
&=\left(\frac{0-1+0-1}{4},\frac{2+2+3+3}{4}\right)
 =(\mathbf{-0.5},\mathbf{2.5}).
\end{aligned}
\]
\end{answerprose}
...
The statement ``75\% are made by Bibi'' supplies the priors
$P(B)=0.75$ and $P(A)=1-0.75=0.25$. For a continuous observation, the values
$f(x\mid A)$ and $f(x\mid B)$ are densities; Bayes' rule combines each density with its
prior and then normalizes:
\[
P(B\mid x=1.1)
...
\closequestion
\begin{answerprose}{0.25}
Bayes' rule for a mixture component is
\[
P(j\mid x)
=\frac{\overbrace{P(j)}^{\text{prior}}\,
       \overbrace{p(x\mid j)}^{\text{component likelihood}}}
      {\underbrace{p(x\mid\theta)}_{\text{total evidence}}}.
\]
The joint contribution of component $j$ and observation $x$ is
$P(j)p(x\mid j)=0.1\cdot0.5=0.05$. Normalizing by the total probability of observing
$x$ gives
\[
P(j\mid x)=\frac{0.05}{0.2}=\mathbf{0.25}.
\]
\end{answerprose}
...
\[
P(j\mid x)=\frac{P(j)\,p(x\mid j)}{p(x\mid\theta)}.
\]
Multiply by $p(x\mid\theta)$ and divide by $P(j)$ to isolate the unknown component
likelihood:
\[
p(x\mid j)
=\frac{P(j\mid x)\,p(x\mid\theta)}{P(j)}
=\frac{0.3\cdot0.2}{0.1}
=\frac{0.06}{0.1}
=\mathbf{0.60}.
\]
\end{answerprose}
...
\[
\alpha_k f(x;\mu_k,\sigma_k)
\]
is the joint contribution of component $k$ and observation $x$. The posterior
responsibility of the first Gaussian is its contribution divided by the sum of the
...
The support of $p_Y$ is $[0,\theta_Y]$. Because $\theta_Y=e^4<e^{10}=\theta_Z$, it lies
entirely inside the support of $p_Z$. On $[0,\theta_Y]$ the two uniform densities are
$1/\theta_Y$ and $1/\theta_Z$, so their ratio is constant:
\[
\begin{aligned}
D_{\mathrm{KL}}(p_Y\|p_Z)
&=\int_0^{\theta_Y}\frac{1}{\theta_Y}
...
For discrete distributions, replace the integral in the KL definition by a sum over all
possible outcomes:
\[
...
\[
\begin{aligned}
&=p(0)\ln\frac{p(0)}{q(0)}+p(1)\ln\frac{p(1)}{q(1)}\\
&=0.8\ln\frac{0.8}{0.5}+0.2\ln\frac{0.2}{0.5}\\
&\approx0.8(0.470)+0.2(-0.916)\\
&=0.376-0.183\approx\mathbf{0.19}.
\end{aligned}
\]
The second outcome contributes a negative term, but the sum is nonnegative, as every KL
divergence must be.
\end{answerprose}
...
\begin{enumerate}[leftmargin=2.2em,itemsep=1.5pt,topsep=2pt]
  ...
  \item Remaining distances:
        ...
        $d(\{h,i,m,n\},\{s\})=\tfrac{11+10+6+5}{4}=8$. The third distance is
        $d(\{a,c,e\},\{s\})=\tfrac{18+16+14}{3}=16$. The smallest remaining
        inter-cluster distance is therefore $8>7$, so we \textbf{stop}.
\end{enumerate}
