...
\begin{defbox}[title={Fisher Information}]
The Fisher information of the parameter $\wvec$ carried by the data is the
variance of the score (the gradient of the log-likelihood):
\[
  \mathcal{I}(\wvec)
  = \E\!\left[\left(\pd{}{\wvec}\ln p(\xvec;\wvec)\right)^{\!2}\right]
  = -\,\E\!\left[\pdd{}{\wvec}\ln p(\xvec;\wvec)\right].
\]
It measures the \emph{curvature} of the log-likelihood at its peak: a sharp,
narrow peak means high information.
For $l$ iid samples the information simply adds up: $\mathcal{I}_l(\wvec) = l\,\mathcal{I}(\wvec)$.
\end{defbox}

\begin{thmbox}[title={Cram\'{e}r--Rao Lower Bound}]
For any \emph{unbiased} estimator $\what$,
\[
  \Var(\what) \;\geq\; \frac{1}{\mathcal{I}(\wvec)}.
\]
...
Almost every objective in this course is secretly an information-theoretic
quantity.
PCA keeps the most ``information'' after projection; ICA drives mutual
information to zero; EM, $t$-SNE, NMF, and Boltzmann machines all minimise a
