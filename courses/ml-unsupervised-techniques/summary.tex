...
Factor analysis (FA) describes the \textbf{variability} of observations in terms of
unobserved \textbf{latent variables} called \emph{factors} and \textbf{noise}.
The factors explain the correlations between observed variables; the remaining
per-variable variance is explained by independent Gaussian noise.
FA is a \emph{generative} approach that models both the noise and the correlations
explicitly.
...
Empirically, with sample covariance $\bm{C} = \frac{1}{n}\bm{X}\T\bm{X}$:
\[
  \bm{C} \;=\; \bm{\Lambda}\bm{\Lambda}\T + \bm{\Psi}.
\]
...
\paragraph{Rotation non-uniqueness}
The FA loading matrix is \textbf{not unique} up to orthogonal rotation:
...
\begin{itemize}
  \item \textbf{Varimax}: maximizes the squared loadings of each factor across all
        variables, so each factor has either large or small loadings --- each variable
        is clearly associated with one factor.
  \item \textbf{Quartimax}: minimizes the number of factors needed per variable;
        each factor explains many variables but is usually hard to interpret.
...
\begin{tabular}{lll}
  ...
  Uniqueness & Unique up to rotation & PCA: unique (up to sign); ICA: unique \\
...
FA's generative model assumes the latent factors $\bm{z}$ are Gaussian; it can
only model the global covariance structure $\bm{C}=\bm{\Lambda\Lambda}\T+\bm{\Psi}$
and cannot distinguish the individual non-Gaussian sources.
The FA scatter plots show a much worse demixing — the tissue subgroups blur
together.

\begin{intbox}[title={Why FA fails for non-Gaussian sources}]
FA is the maximum-likelihood model when factors are Gaussian.
Forcing super-Gaussian data into a Gaussian-factor model distorts the latent
structure: the EM algorithm converges, but to a solution that explains the
observed covariance by smearing mass along incorrect axes.
ICA is the right tool whenever the true sources are non-Gaussian.
\end{intbox}
...
Applying FA to the Iris data set with only \textbf{one factor} is sufficient to
achieve a clean, linear separation of all three species (setosa, versicolor,
virginica).
The single maximum-likelihood factor captures the dominant axis of biological
variation; a scatter plot of the factor scores colour-coded by species shows
three nearly non-overlapping clusters arranged along a line.
...
All dimensionality-reduction methods in this chapter project data to a
low-dimensional space, sometimes called \emph{scaling} of the data.
The goal is either to \textbf{visualise} the data or to produce a compact
low-dimensional representation for downstream tasks, exploiting the fact that
low-dimensional models are simpler and that noise and outliers are not
...
All three are minimised by gradient descent.
...
The non-negativity constraint forces a \textbf{parts-based representation}:
...
\]
The constraint makes the weights invariant to rotations, rescalings, and
translations of $\bm{x}_i$.
Define the local covariance $C_{jk} = \bm{x}_j\T\bm{x}_k$; the Lagrange
multiplier enforcing the sum-to-one constraint is:
\[
  \lambda = \frac{1 - \sum_{jk} C_{jk}^{-1}\,\bm{x}_i\T\bm{x}_k}
                 {\sum_{jk} C_{jk}^{-1}},
\]
and the optimal weights are:
\[
  W_{ij} = \sum_k C_{jk}^{-1}\bigl(\bm{x}_i\T\bm{x}_k + \lambda\bigr).
\]
...
\]
which can be written compactly as $\bm{M}=(\bm{I}-\bm{W})(\bm{I}-\bm{W})\T$.
Because the row sums of $\bm{W}$ are one, $(\bm{I}-\bm{W})\T\mathbf{1}=\mathbf{0}$,
...
latent space:
\[
  p(\bm{t}\mid\bm{w},\beta) = \int p(\bm{t}\mid\bm{x},\bm{w},\beta)\,p(\bm{x})\,\mathrm{d}\bm{x}.
\]
...
\textbf{Iris data}: PCA partially overlaps versicolor and virginica; t-SNE
achieves perfect three-cluster separation with no overlap.
...
\begin{enumerate}
  \item Find the \emph{winner} neuron:
        $k^* = \arg\max_s\;\bm{x}\T\bm{w}_s$.
...
The SOM update rule cannot be derived from a single scalar energy or
loss function.
Practical consequences:
\begin{itemize}
  \item No theoretical basis for choosing $\eta$, neighbourhood width, or
        stopping criterion.
  \item No guarantee that topographic ordering is achieved.
  \item No convergence proof.
  \item Different runs cannot be compared objectively.
  \item Overfitting is not detectable.
...
$k$-means is obtained from a mixture of Gaussians by imposing three
simplifying assumptions:
\begin{enumerate}
  \item equal weights $w_j = 1/l$;
  \item spherical and equal covariance $\bm{\Sigma}_j^{-1} = \bm{I}$;
...
\textbf{Properties}: $k$-means is fast and robust to outliers (hard assignment
prevents a single outlier from shifting all means), but it is simple (only
spherical, equal-size clusters) and sensitive to initialisation.
A centre placed near outliers can remain stuck there,
failing to model the true cluster distribution.

\textbf{Fuzzy $k$-means} replaces the hard membership with a soft
(``softmax'') membership controlled by a parameter $b > 1$:
\[
  p^b(j\mid\bm{x}_i,\bm{\mu}_j)
...
\[
  \bm{\mu}_j^{\text{new}}
  = \frac{\sum_i p^b(j\mid\bm{x}_i,\bm{\mu}_j)\,\bm{x}_i}
         {\sum_i p^b(j\mid\bm{x}_i,\bm{\mu}_j)},
\]
and the minimised objective is
$\sum_j\sum_i p^b(j\mid\bm{x}_i,\bm{\mu}_j)\,\|\bm{x}_i-\bm{\mu}_j\|^2$.
...
\begin{intbox}[title={Linkage behaviour}]
...
\begin{itemize}
  ...
  \item \textbf{M-step}: update the parameters using the weighted counts
        ...
        \begin{align*}
          ...
          p(y\mid z) &= \frac{\sum_x n(x,y)\,p(z\mid x,y)}{p(z)},\\
          p(x\mid z) &= \frac{\sum_y n(x,y)\,p(z\mid x,y)}{p(z)}.
        \end{align*}
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Preferences} $s(k,k)$: the prior probability that object $k$
        becomes an exemplar; a uniform value controls the expected number of
...
\textbf{Limitation.} AP implicitly assumes spherical clusters of equal size
because similarities are typically Euclidean distances.
When cluster sizes differ, AP cannot adjust the effective bandwidth per
cluster, and elements of a large diffuse cluster may be assigned to a nearby
compact cluster instead.
...
\begin{enumerate}
  ...
  \item \textbf{Weight update}: $\alpha_i^{\text{new}} = \dfrac{n\hat\alpha_i + \gamma_i - 1}{\gamma_s}$.
...
A bicluster is the rank-1 matrix $\mathbf{u}_j \mathbf{y}_j^T$ where
\begin{itemize}
...
\[
  X = \sum_{j=1}^{l} \mathbf{u}_j \mathbf{y}_j^T + \Gamma = Y U^T + \Gamma,
\]
...
The resulting sparse outer products $\mathbf{u}_j\mathbf{y}_j^T$ cleanly delimit rectangular
...
maxima; \emph{deterministic annealing} (used in HMMER) avoids this.
...
\begin{itemize}
  ...
  \item Cannot handle real-valued inputs (discrete alphabet only).
...
state into several parallel chains $u^1, u^2, \ldots, u^h$ (three in the standard
picture). The transition of chain $u^i$ is conditioned on the chains $u^k$ with
$k \le i$, and the emission depends jointly on \emph{all} chains. A natural
reading is multi-timescale dynamics: $u^1$ evolves slowly, the higher chains
faster, with slow chains influencing fast ones but not the reverse.
...
pervasive: Pfam is built entirely on them, with roughly 67\% of proteins
containing at least one Pfam profile and about 45\% of all residues covered. The
same machinery, outside the profile setting, is also used for tasks like
...
How does a unit decide its state? Flipping unit $i$ from off to on changes the
energy by
\[
...
\[
  p_{i=\text{on}} = \frac{1}{1 + \exp(-\Delta E_i / T)} ,
\]
...
out of poor local minima — and then gradually cools, so the energy settles near
a global minimum while still fluctuating a little around it. This is simulated
annealing.
...
\end{itemize}
The gradient of the divergence with respect to a weight turns out to be
beautifully simple — just the difference of these two correlations:
\[
  \pd{D}{w_{ij}} = -\frac{1}{\eta}\left( p^+_{ij} - p^-_{ij} \right),
  \qquad
  \pd{D}{\theta_i} = -\frac{1}{\eta}\left( p^+_{i} - p^-_{i} \right),
\]
...
machine impractical:
\begin{itemize}
  \item the time needed to collect equilibrium statistics grows
        \emph{exponentially} with the size of the machine and the magnitude of
        the weights;
