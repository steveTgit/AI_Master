...
\begin{itemize}[leftmargin=1.8em]
  ...
  \item Two row-sums equal $1$ and the rest equal $2$ (a single open path with two endpoints). \hfill\answerline{3cm}
...
\begin{enumerate}[label=\textbf{Q\arabic*.}, leftmargin=3em, itemsep=4pt]
  ...
  \item Single $6$-cycle (all degrees $2$, one cycle) $\to$ $A$ (and the differently-drawn cycle $\to C$);
        open path (two degree-$1$ endpoints, so not $B$) $\to$ \emph{none of the above}; two separate $3$-cycles $\to$ $B$;
        cycle with one chord (two degree-$3$ nodes) $\to$ \emph{none of the above}. The shown $D$ instead has
        one degree-$1$ endpoint and one degree-$3$ attachment node. Match each matrix by its row-sums
        (node degrees) and whether the edges form one cycle, two triangles, or a cycle-plus-chord.
