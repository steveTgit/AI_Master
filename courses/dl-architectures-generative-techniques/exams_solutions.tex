...
A ground-truth object covers a set of pixels and a model predicts another set of pixels for that
object, on a grid of pixels. The ground-truth region covers $10$ pixels, the predicted region
covers $8$ pixels, and they overlap in $5$ pixels. Compute the IoU metric, rounded to three
decimals.
\closequestion
...
\begin{itemize}[leftmargin=1.8em]
  ...
  \item Two row-sums equal $1$ and the rest equal $2$ (an open path, not a cycle).
...
\end{itemize}
\closequestion
\begin{answerprose}{$A/C$ \ / \ none \ / \ $B$ \ / \ $D$}
...
\begin{itemize}[leftmargin=1.7em,itemsep=1.5pt,topsep=2pt]
  ...
  \item[\textcolor{ok}{$\to$}] Two degree-$1$ nodes (open path) $\Rightarrow$ \textbf{none of the above} (no shown graph is an open path).
  \item[\textcolor{ok}{$\to$}] All degrees $2$ but split into two $3$-cycles $\Rightarrow$ \textbf{$B$} (two triangles).
  \item[\textcolor{ok}{$\to$}] Two degree-$3$ nodes (cycle $+$ chord) $\Rightarrow$ \textbf{$D$}.
\end{itemize}
\end{answerprose}
